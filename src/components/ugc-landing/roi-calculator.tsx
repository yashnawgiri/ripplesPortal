...
export function ROICalculator() {
  ...
  const calculateMetrics = () => {
    ...
    const totalConversionLift = Math.min(baseConversionLift + additionalLift, 0.3)

    const cashbackCost = state.avgOrderValue * (state.cashbackPercent / 100) * monthlyParticipants
    const additionalSales = state.monthlyOrders * totalConversionLift * state.avgOrderValue
        ...
        <div className="grid gap-6">
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <EnhancedInput
              ...
              onChange={(e) => handleInputChange("cashbackPercent", e.target.value)}
            />
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card className="bg-white/5 border-white/10 transform transition-all duration-300 hover:scale-105 hover:bg-white/10">
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-white">{metrics.monthlyUGCPieces}</div>
                <p className="text-xs text-white/60">Monthly UGC Pieces</p>
              </CardContent>
            </Card>
            <Card className="bg-white/5 border-white/10 transform transition-all duration-300 hover:scale-105 hover:bg-white/10">
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-white">${metrics.cashbackCost.toLocaleString()}</div>
                <p className="text-xs text-white/60">Monthly Cashback Cost</p>
              </CardContent>
            </Card>
            <Card className="bg-white/5 border-white/10 transform transition-all duration-300 hover:scale-105 hover:bg-white/10">
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-white">${metrics.additionalSales.toLocaleString()}</div>
            ...
            <Card className="bg-white/5 border-white/10 transform transition-all duration-300 hover:scale-105 hover:bg-white/10">
              <CardContent className="pt-6">
                <div className="text-2xl font-bold text-white">{metrics.conversionLift}%</div>
